...
# 진단 신호: 429는 곧 "아직 Spark"
- **운영 로그에 Firestore `429 RESOURCE_EXHAUSTED Quota exceeded`가 보이면 = 해당 프로젝트가 아직 무료(Spark) 상태.** Blaze(종량제)는 일일 무료 한도를 넘겨도 막지 않고 과금만 하므로 이 429 자체가 안 난다. 사용자가 "Blaze 업그레이드 했다"는데 429가 계속되면 **다른 프로젝트에 적용했거나 결제 연결 미완료**를 의심하고, Firebase Console 좌하단 요금제 배지가 `Blaze`인지 + 정확히 `geonseolup` 프로젝트인지 확인시킬 것.
- **Why:** 코드 디버깅으로 풀 문제가 아님(증상은 발행 중단·목록 빈 화면·"서버 연결 확인 중" 전부 동일 뿌리). Blaze가 올바른 프로젝트에 걸리는 순간 429가 사라지고 로그가 정상 발행/중복방지 건너뜀으로 바뀐다.
