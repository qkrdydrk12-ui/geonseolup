...
// ── 직종 동의어 정규화 ──────────────────────────────────────────────────────
export type JobSynonymMap = Record<string, string[]>;

export const DEFAULT_JOB_SYNONYMS: JobSynonymMap = {
  '화기감시자': ['화재감시자', '화재감시', '화기감시', '화재 감시', '화기 감시', '화재 감시자'],
  '조공': ['잡부', '노무'],
  '용접': ['용접공', '웰딩'],
};

export function loadJobSynonyms(): JobSynonymMap {
  try {
    const stored = localStorage.getItem('cj_job_synonyms');
    if (stored) return JSON.parse(stored) as JobSynonymMap;
  } catch {}
  return { ...DEFAULT_JOB_SYNONYMS };
}

export function saveJobSynonyms(map: JobSynonymMap): void {
  localStorage.setItem('cj_job_synonyms', JSON.stringify(map));
}

function buildReverseLookup(map: JobSynonymMap): Record<string, string> {
  const reverse: Record<string, string> = {};
  for (const [canonical, synonyms] of Object.entries(map)) {
    for (const syn of synonyms) {
      reverse[syn.trim()] = canonical;
    }
  }
  return reverse;
}

export function normalizeJob(input: string): string {
  const trimmed = input.trim();
  if (!trimmed) return trimmed;
  const map = loadJobSynonyms();
  const reverse = buildReverseLookup(map);
  return reverse[trimmed] ?? trimmed;
}

export function getJobAliases(canonical: string): string[] {
  const map = loadJobSynonyms();
  const synonyms = map[canonical] || [];
  return [canonical, ...synonyms];
}

export function jobMatchesKeyword(job: string, kw: string): boolean {
  const normalized = kw.toLowerCase();
  if (job.toLowerCase().includes(normalized)) return true;
  const aliases = getJobAliases(job);
  return aliases.some((a) => a.toLowerCase().includes(normalized));
}
