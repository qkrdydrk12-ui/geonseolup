      ...
      <table className="w-full border-collapse min-w-[520px]">
        <thead>
          <tr className="border-b border-gray-100">
            ...
            <th className={sortableThCls} onClick={() => onSortChange('likes')}>❤️ 좋아요{arrow('likes')}</th>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.key} className={`border-b border-gray-50 last:border-0 ${r.lowView ? 'bg-red-50/40' : ''}`}>
              ...
              <td className="px-3 py-2 align-top text-sm font-semibold text-gray-700 whitespace-nowrap">{r.likes.toLocaleString()}</td>
              <td className="px-3 py-2 align-top">
