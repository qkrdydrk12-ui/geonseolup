export interface RankedBarItem {
  label: string;
  value: number; // 조회수·이탈수·초 등 — 의미는 title/section 문맥에서 정해짐
}

// 지역별/직종별 조회수, 이탈 지점, 페이지별 체류시간처럼 "항목 + 막대 + 숫자"를 보여주는
// 간단한 공통 리스트. 별도 차트 라이브러리 없이 순수 CSS 막대(비율 폭)로 표현한다.
export default function AdminRankedBars({ title, items, limit = 8 }: { title: string; items: RankedBarItem[]; limit?: number }) {
  const top = items.slice(0, limit);
  const max = top.length ? Math.max(...top.map((i) => i.value), 1) : 1;
  return (
          ...
          {top.map((item, i) => (
            <div key={item.label} className="flex items-center gap-2">
              ...
              <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full bg-[#f97316] rounded-full" style={{ width: `${Math.max((item.value / max) * 100, item.value > 0 ? 3 : 0)}%` }} />
              </div>
              <span className="text-[11px] font-bold text-gray-600 w-10 text-right shrink-0">{item.value.toLocaleString()}</span>
            </div>
          ))}
        </div>
      )}
